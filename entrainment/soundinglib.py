...
#   current_axes.set_position([0, 0, 1, 1])
    ...
    _stlp_axes_transform = _stlp_data_transform + current_axes.transAxes.inverted()
    ...
    for p_line in range(_p_min, _p_max + _p_step, _p_step):
        ...
        ax_x, ax_y = (_stlp_ylabel_transform + current_axes.transAxes.inverted()).transform(np.array([[0, p_line]]))[0]
        if ax_x >= 0 and ax_x <= 1 and ax_y >= 0 and ax_y <= 1:
    ...
    for T_line in range(_T_plot_min, _T_max + _T_step, _T_step):
        ...
        ax_x, ax_y = _stlp_axes_transform.transform(np.array([[T_line, 1000]]))[0]
        if ax_x >= 0 and ax_x <= 1 and ax_y >= 0 and ax_y <= 1:
...
def computeParcelTrace(t_sfc, td_sfc, p_snd):
    r_sfc = mixingRatio(saturationVaporPressure(td_sfc + 273.15), p_snd[0] * 100)

    lcl_temp, lcl_pres, lcl_r = iterativelib.solveIteratively(iterativelib.condensationTemp, [t_sfc + 273.15, p_snd[0] / 10., r_sfc])
    lcl_idx = np.argmin(np.abs(lcl_pres * 10 - p_snd))

    if p_snd[lcl_idx] > lcl_pres * 10:
        lcl_idx += 1

    dry_adiabatic_pres = np.append(p_snd[:lcl_idx], lcl_pres * 10)
    moist_adiabatic_pres = np.append([ lcl_pres * 10 ], p_snd[lcl_idx:])

    dry_adiabatic_temp = (t_sfc + 273.15) * (dry_adiabatic_pres / p_snd[0]) ** (2. / 7.) - 273.15
    moist_adiabatic_temp = odeint(moistAdiabaticLapseRate, lcl_temp, 100 * moist_adiabatic_pres)[:, 0] - 273.15

    return np.append(dry_adiabatic_temp[:-1], moist_adiabatic_temp), np.append(dry_adiabatic_pres[:-1], moist_adiabatic_pres)
